...
import view.drawing.DrawingArea;
...
public class BSTVisualizer {
	...
	private void calculateWidth(VNode root) {
		if(root.node == null) {
			...
			calculateWidth(root.left  = new VNode(root.node.getLeft()));
			calculateWidth(root.right = new VNode(root.node.getRight()));
			root.width = 1 + root.left.width + root.right.width;
		}
	}

	private VNode buildVTree(BinarySearchTree<?> bst) {
		VNode root = new VNode(bst.getRoot());
		buildVTree(root);
		return root;
	}
	...
	private void buildVTree(VNode root) {
		if(root.node != null) {
			root.left = new VNode(root.node.getLeft());
			root.right = new VNode(root.node.getRight());
		}
	}
	...
	private void drawTree(VNode vnode, int level, int offset) {
		...
		String text = String.valueOf(vnode.node.getValue());
